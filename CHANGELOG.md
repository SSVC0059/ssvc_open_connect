...
### Новое в версии
  ...
  - Новые зоны датчиков: `tank` (куб), `outdoor` (атмосфера)
  - Модальное окно авторасчёта порогов давления в настройках датчика
  - Отображение давления в баке на панели телеметрии
  - Docker-контейнер для сборки (`Dockerfile`, `.dockerignore`)
  - Версия API повышена до `SSVC_SUPPORT_API_VERSION=1.7`

### Исправление ошибок

  - `SpeedSettings`: исправлены ключи `onSave` для параметров отбора подголовников, тела и хвостов
  - Мастер настроек: Значение в -1 у поля "Время сброса" в мастере настроек блокирует запуск ректификации [#75](https://github.com/SSVC0059/ssvc_open_connect/issues/75)
  - CoreDump: потоковая выгрузка чанками, 404 при отсутствии дампа, очистка только после полной передачи
  - Ошибка при одновременном подключении нескольких датчиков ds18b20 [#107](https://github.com/SSVC0059/ssvc_open_connect/pull/107)
  - Убрано ручное освобождение mutex [#108](https://github.com/SSVC0059/ssvc_open_connect/pull/108)
  - Переполнение chat_id при использовании String::toInt() — ID > 2147483647 [#109](https://github.com/SSVC0059/ssvc_open_connect/issues/109)


### Стабильность [#111](https://github.com/SSVC0059/ssvc_open_connect/issues/111)
  - Исправление инициализации openConnect в случае когда esp32 включена раньше чем SSVC, 
  - Восстановление работы при обрыве UART соединения
  - Индикация ошибки UART: Wled светодиод мигает оранжевым при потери соединения
  - Отправка сообщения в TG при потери соединения

### Тестирование и CI

  - Переход на `vitest run`, job `native-tests` (`pio test -e native`) в CI
  - Добавлены `@testing-library/*` для компонентных и интеграционных тестов

### Сборка и релизы

  - Ночная сборка: реализована отдельная сборочная линия для публикации ночного релиза с тегом вида v<ВЕРСИЯ_СБОРКИ>-nightly

### Изменено

  - Обновление лицензии: код — GPL v3, документация и дизайн — CC BY-SA 4.0
  - Стили: переменные цветов, breakpoints, z-index, тёмная тема для профилей и настроек


### Frontend

  - Унификация стилей: консолидация CSS‑переменных в app.css, исправления тёмной темы, миграция на DaisyUI
  - Удалена зависимость Flowbite; tooltip, loading и popover заменены на компоненты DaisyUI
  - Реструктуризация маршрутов: вкладки для /system (статус, метрики, coredump, обновление), /wifi (STA/AP), /network (MQTT/NTP), /oc/settings, /oc/ssvc
  - Раздел «Соединения» перенесён в /network с вкладками MQTT и NTP
  - Обновление меню: отдельные иконки для SSVC и профилей, исправлена структура пунктов
  - Добавлены видимые индикаторы фокуса для доступности (a11y)
  - Включена стратегия single bundle для SvelteKit

### Документация

  - Добавлен скрипт автоматической генерации кнопок автофлешера по актуальным релизам
  - Автофлешер обновлен на установку последней стабильной и тестовой версии прошивки
  - Добавлена возможность прошивки через выбор файла с диска
