...
### Новое в версии
  ...
  - Модель данных сенсоров: payload изменён с `number` на `{ v, u, type }`, фильтрация и отображение по типу
  - Новые зоны датчиков: `tank` (куб), `outdoor` (атмосфера)
  - Модальное окно авторасчёта порогов давления в настройках датчиков
...
### Исправлено
  ...
  - Мастер настроек: поле «Время сброса» больше не открывается как `-1` [#75](https://github.com/SSVC0059/ssvc_open_connect/issues/75)
  - CoreDump: потоковая выгрузка чанками, 404 при отсутствии дампа, очистка только после полной передачи
  - Замена `delay` на `vTaskDelay` [#107](https://github.com/SSVC0059/ssvc_open_connect/pull/107)
