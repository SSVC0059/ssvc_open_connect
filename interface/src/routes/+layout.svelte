<script lang="ts">
	...
	import { processIncomingData  } from '$lib/stores/ssvcOpenConnect';
	import { processTelemetryData } from '$lib/utils/ssvcHelper';
	...
	// 	SSVC
		...
		socket.on<SsvcOpenConnectMessage>("openConnect", (data) => {
			// Записываем обработанные данные в store
	...
	const removeEventListeners = () => {
		...
		socket.off("openconnect", handleCloseOpenConnect);
	};
	...
	const handleCloseOpenConnect = () => {
		notifications.success('Соединение с сокетом OpenConnect разорвано', 5000);
	};
