<script lang="ts">
	...
	const unsubscribeSsvc = ssvcTelemetryStore.subscribe((value) => {
		if (value) {
			...
			console.log("ssvcMessage.type: " + ssvcMessage.type);
	...
	<div class="container">
		...
		<div>
			<div class="row flex mt-2">
				<div class="col ">
					<div class="item bg-gray-100 dark:bg-opacity-25 border border-black rounded-lg shadow-sm mx-1">
			...
			<div class="row bg-gray-100 dark:bg-opacity-25 rounded-lg">
				<div class="col-6 flex justify-center mx-1 item ">
					<div class="relative min-w-full">
						<img src={RectImg} alt="Rect" class="object-contain mx-auto  max-w-full h-auto" />
						...
						{/if}
						{#if ssvcMessage.countdown}
							<div id="countdown" class="absolute top-2 left-2 bg-white bg-opacity-40 dark:bg-opacity-0">
								<TimeBlock  name="" timeString="{ssvcMessage.countdown}" />
								<Popover placement = 'right'  class="w-64 text-sm font-light text-gray-500 bg-white dark:text-gray-400 dark:border-gray-600 dark:bg-gray-800"
													transition={slide} triggeredBy="#countdown" >
						...
						{#if ssvcMessage.time}
							<div id="time"  class="absolute top-2 right-0 bg-white bg-opacity-40 dark:bg-opacity-0">
								<TimeBlock name="" timeString="{ssvcMessage.time}" />
								<Popover placement = 'left'  class="w-64 text-sm font-light text-gray-500 bg-white dark:text-gray-400 dark:border-gray-600 dark:bg-gray-800"
												 transition={slide} triggeredBy="#time" >
									Пройденное время этапа
								</Popover>
							</div>
						{/if}
						<div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/5 bg-white bg-opacity-40 dark:bg-opacity-0">
