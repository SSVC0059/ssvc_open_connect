...
- **Сейчас:** в трёх компонентах используются Flowbite-svelte (Popover, Spinner, иконки). Новый функционал предпочтительно реализовывать без добавления зависимостей от Flowbite.
- При доработке этих компонентов рекомендуется заменять Flowbite на DaisyUI/кастомные решения (dropdown, loading-состояния и т.д.).
