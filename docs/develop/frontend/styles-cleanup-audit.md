...
2. **Внешний вид вкладок**
   - **OC → SSVC → Настройки** (`/oc/ssvc`): блок с вкладками на DaisyUI (`tabs tabs-lift` + radio‑элементы `input[type="radio"][role="tab"]`), активная вкладка с фоном `var(--color-primary)` и нижней границей, общая нижняя граница задана глобально в `app.css`.
   - **OC → Настройки** (`/oc/settings`): вкладки «Профили», «Telegram» и др. используют такой же паттерн DaisyUI (`tabs tabs-lift`, `.tab`, `.tab-content`), что и `/oc/ssvc`, `/network` и `/system`.
   - Тёмная тема: цвета заголовков и пунктов внутри `.tabs-container` (профили, подписи, заголовки настроек) должны соответствовать актуальным правилам для темизации (семантика DaisyUI / `oklch(var(--bc))`), без зависимости от устаревших длинных селекторов из `_dark-theme.scss`.
