...
private:
  ...
  std::tuple<float, int> parallel = {
      0.0, 0}; // параллель (диапазон для параметра параллели)
  std::tuple<float, int> parallel_v1 = {
      0.0, 0}; // Скорость параллельного отбора клапаном №1
  std::tuple<float, int> parallel_v3 = {
      0.0, 0}; // Скорость параллельного отбора клапаном №3 для 4-х диапазонов
