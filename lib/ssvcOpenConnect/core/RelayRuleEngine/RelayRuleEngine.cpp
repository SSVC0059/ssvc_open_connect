...
void RelayRuleEngine::setManualOverride(const unsigned bit, const bool energized, const bool enableOverride) {
  ...
  const uint8_t b = static_cast<uint8_t>(1u << bit);
  if (RelayPortCoordinator::getInstance().reservedAlarmBitsMask() & b) {
    return;
  }
  if (!enableOverride) {
    _manualMask = static_cast<uint8_t>(_manualMask & ~b);
  } else {
    _manualMask = static_cast<uint8_t>(_manualMask | b);
    if (energized) {
      _manualEnergized = static_cast<uint8_t>(_manualEnergized | b);
    } else {
      _manualEnergized = static_cast<uint8_t>(_manualEnergized & ~b);
    }
  }
...
void RelayRuleEngine::appendCapabilitiesJson(JsonObject& root) const {
  root["i2cAddress"] = SSVC_RELAY_PCF8574_I2C_ADDR;
...
void RelayRuleEngine::recomputeAndApply() {
  ...
  const uint8_t umask = coord.userBitsMask();
  for (unsigned bit = 0; bit < 8; ++bit) {
    ...
    bool found = false;
    for (const Rule& r : _rules) {
      if (!r.enabled || r.targetBit != static_cast<int>(bit)) {
        continue;
      }
    ...
    coord.setUserLine(bit, found ? bestEnergize : false);
  }
  coord.flush();
#endif
}
